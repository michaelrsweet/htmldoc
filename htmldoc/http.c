	 ...
	 size_t length)			/* I - Maximum number of bytes */
{
  ...
  DEBUG_printf(("httpPeek(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")",
	  ...
	  size_t length)		/* I - Maximum number of bytes */
{
  ...
  DEBUG_printf(("httpRead2(http=%p, buffer=%p, length=" HTMLDOC_LLFMT
                ") coding=%d data_encoding=%d data_remaining=" HTMLDOC_LLFMT,
                http, buffer, HTMLDOC_LLCAST length,
                http->coding,
                http->data_encoding, HTMLDOC_LLCAST http->data_remaining));
#else
  DEBUG_printf(("httpRead2(http=%p, buffer=%p, length=" HTMLDOC_LLFMT
                ") data_encoding=%d data_remaining=" HTMLDOC_LLFMT,
                http, buffer, HTMLDOC_LLCAST length,
                http->data_encoding, HTMLDOC_LLCAST http->data_remaining));
#endif /* HAVE_LIBZ */
		...
		size_t urilen)		/* I - Size of URI buffer */
{
  ...
  */
  ...
  DEBUG_printf(("httpReadRequest(http=%p, uri=%p, urilen=" HTMLDOC_LLFMT ")",
                http, uri, HTMLDOC_LLCAST urilen));
  ...
  switch (field)
  {
    case HTTP_FIELD_ACCEPT_ENCODING :
        if (http->default_accept_encoding)
          free(http->default_accept_encoding);

        http->default_accept_encoding = value ? strdup(value) : NULL;
        break;

    case HTTP_FIELD_SERVER :
        if (http->default_server)
          free(http->default_server);

        http->default_server = value ? strdup(value) : NULL;
        break;

    case HTTP_FIELD_USER_AGENT :
        if (http->default_user_agent)
          free(http->default_user_agent);

        http->default_user_agent = value ? strdup(value) : NULL;
        break;
	     ...
	     const char   *value)	/* I - Value */
{
  ...
  switch (field)
  {
    case HTTP_FIELD_ACCEPT_ENCODING :
        ...
        http->accept_encoding = strdup(value);
        break;
...
{
  DEBUG_printf(("httpSetLength(http=%p, length=" HTMLDOC_LLFMT ")", http,
  ...
  */
  ...
  if (!http->server)
    httpSetField(http, HTTP_FIELD_SERVER,
                 http->default_server ? http->default_server : "HTMLDOC/" SVERSION);
          ...
          size_t length)		/* I - Maximum bytes to read */
{
  ...
  DEBUG_printf(("http_read(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")", http,
                buffer, HTMLDOC_LLCAST length));
                   ...
                   size_t length)	/* I - Maximum bytes to read */
{
  ...
  DEBUG_printf(("http_read_buffered(http=%p, buffer=%p, length=" HTMLDOC_LLFMT
                ") used=%d",
                http, buffer, HTMLDOC_LLCAST length, http->used));
...
http_read_chunk(http_t *http,		/* I - HTTP connection */
		...
		size_t length)		/* I - Maximum bytes to read */
{
  DEBUG_printf(("http_read_chunk(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")",
                http, buffer, HTMLDOC_LLCAST length));
  ...
  */
  ...
  if (!http->tls)
  {
    ...
    */
    ...
    _cupsSetError(IPP_STATUS_ERROR_HTMLDOC_PKI, _("Encryption is not supported."), 1);
    httpAddrClose(NULL, http->fd);
	   ...
	   size_t     length)		/* I - Number of bytes to write */
{
  ...
  DEBUG_printf(("2http_write(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")", http,
                buffer, HTMLDOC_LLCAST length));
  http->error = 0;
  tbytes      = 0;
		 ...
		 size_t        length)	/* I - Length of buffer */
{
  ...
  DEBUG_printf(("7http_write_chunk(http=%p, buffer=%p, length=" HTMLDOC_LLFMT ")",
                http, buffer, HTMLDOC_LLCAST length));
