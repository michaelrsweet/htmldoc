...
GUI::GUI(const char *filename)		// Book file to load initially
{
  ...
  browserWidth->value(680.0 /*_htmlBrowserWidth*/); // TODO
...
GUI::loadSettings()
{
  ...
  sprintf(s, "%.1fpt", fontBaseSize->value());
  _htmlStyleSheet->set_font_size(s);
  sprintf(s, "%.0f%%", 100.0 * fontSpacing->value());
  _htmlStyleSheet->set_line_height(s);
  ...
  _htmlStyleSheet->set_color(textColor->value());
  _htmlStyleSheet->set_charset(charset->text(charset->value()));

  strlcpy(LinkColor, linkColor->value(), sizeof(LinkColor));
  LinkStyle = linkStyle->value();

  _htmlStyleSheet->browser_width = browserWidth->value();
...
GUI::newBook(void)
{
  ...
  fontBaseSize->value(_htmlStyleSheet->def_style.font_size);
  fontSpacing->value(_htmlStyleSheet->def_style.line_height /
                     _htmlStyleSheet->def_style.font_size);
  headFootSize->value(HeadFootSize);

  for (i = 0; i < (charset->size() - 1); i ++)
    if (strcasecmp(_htmlStyleSheet->charset, charset->text(i)) == 0)
    {
      charset->value(i);
      break;
    }
  ...
  if (Permissions & HD_PDF_PERM_PRINT)
    permPrint->set();
  else
    permPrint->clear();

  if (Permissions & HD_PDF_PERM_MODIFY)
    permModify->set();
  else
    permModify->clear();

  if (Permissions & HD_PDF_PERM_COPY)
    permCopy->set();
  else
    permCopy->clear();

  if (Permissions & HD_PDF_PERM_ANNOTATE)
    permAnnotate->set();
  else
    permAnnotate->clear();
  ...
  path->value(Path);
  proxy->value(Proxy);
  browserWidth->value(_htmlStyleSheet->browser_width);
  strict_html->value(StrictHTML);
  overflow_errors->value(OverflowErrors);
