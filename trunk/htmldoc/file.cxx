//
// "$Id: file.c,v 1.22 2004/10/22 05:43:14 mike Exp $"
//
...
// Contents:
//
//   hdBook::file_basename()    - Return the base filename without directory or target.
//   hdBook::file_cleanup()     - Close an open HTTP connection and remove
//                                temporary files...
//   hdBook::file_cookies()     - Set the HTTP cookies for remote accesses.
//   hdBook::file_directory()   - Return the directory without filename or target.
//   hdBook::file_extension()   - Return the extension of a file without the target.
//   hdBook::file_find_check()  - Check to see if the specified file or URL exists...
//   hdBook::file_find()        - Find a file in one of the path directories.
//   hdBook::file_gets()        - Read a line from a file terminated with CR, LF,
//                                or CR LF.
//   hdBook::file_localize()    - Localize a filename for the new working directory.
//   hdBook::file_method()      - Return the method for a filename or URL.
//   hdBook::file_nolocal()     - Disable access to local files.
//   hdBook::file_proxy()       - Set the proxy host for all HTTP requests.
//   hdBook::file_target()      - Return the target of a link.
//   hdBook::file_temp()        - Create and open a temporary file.
//
...
hdBook::file_basename(const char *s)	// I - Filename or URL
{
...
/*
 * 'hdBook::file_cleanup()' - Close an open HTTP connection and remove temporary files...
 */

void
hdBook::file_cleanup(void)
{
  int		i;			/* Looping var */
...
void
hdBook::file_cookies(const char *s)	// I - Cookie string or NULL
{
  if (s)
...
hdBook::file_directory(const char *s)	// I - Filename or URL
{
...
hdBook::file_extension(const char *s)	// I - Filename or URL
{
...
hdBook::file_find_check(const char *filename)
					// I - File or URL
{
  int		i;			// Looping var
  ...
  DEBUG_printf(("hdBook::file_find_check(filename=\"%s\")\n", filename));
  ...
  if (strcmp(method, "file") == 0)
  {
    ...
    */
    ...
    {
      DEBUG_printf(("hdBook::file_find_check: Returning \"%s\"!\n", filename));
      return (filename);
    }
  }
  else
  {
    ...
    */
    ...
    for (i = 0; i < web_files; i ++)
      ...
      {
        DEBUG_printf(("hdBook::file_find_check: Returning \"%s\" for \"%s\"!\n",
    ...
    if ((fp = hdBook::file_temp(tempname, sizeof(tempname))) == NULL)
    {
      progress_hide();
...
const char *				// O - Pathname or NULL
hdBook::file_find(const char *path,	// I - Path "dir;dir;dir"
          const char *s)		// I - File to find
{
  ...
  DEBUG_printf(("hdBook::file_find(path=\"%s\", s=\"%s\")\n", path ? path : "(null)", s));

  // See if this is a cached remote file...
  for (i = 0; i < web_files; i ++)
    if (strcmp(s, web_cache[i].name) == 0)
    {
      DEBUG_printf(("hdBook::file_find: Returning cache file \"%s\"!\n", s));
      return (s);
    }

  DEBUG_printf(("hdBook::file_find: \"%s\" not in web cache of %d files...\n",
                s, web_files));
  ...
  if (path != NULL && !path[0])
  {
    DEBUG_puts("hdBook::file_find: Resetting path to NULL since path is empty...");
    path = NULL;
  }
  ...
  if (strncmp(s, "http:", 5) == 0 ||
      ...
      strncmp(s, "//", 2) == 0)
  {
    DEBUG_puts("hdBook::file_find: Resetting path to NULL since filename is a URL...");
    path = NULL;
  }
  ...
  if (path != NULL)
  {
	...
	*temp++ = '/';
      ...
      if ((realname = hdBook::file_find_check(filename)) != NULL)
	return (realname);
    }
  }

  return (hdBook::file_find_check(s));
}


//
// 'hdBook::file_gets()' - Read a line from a file terminated with CR, LF, or CR LF.
//

char *					// O - Line from file or NULL on EOF
hdBook::file_gets(char  *buf,		// I - Line buffer
...
hdBook::file_localize(const char *filename,
					// I - Filename
                      const char *newcwd)
					// I - New directory
{
  ...
  if (hdBook::file_method(filename))
    return (filename);
...
hdBook::file_method(const char *s)	// I - Filename or URL
{
  if (strncmp(s, "http:", 5) == 0)
    return ("http");
...
void
hdBook::file_proxy(const char *url)	// I - URL of proxy server
{
...
hdBook::file_target(const char *s)	// I - Filename or URL
{
  const char	*basename;		// Pointer to directory separator
...
hdBook::file_temp(char   *name,		// O - Filename
                  size_t len)		// I - Length of filename buffer
{
  hdFileCache	*temp;			// Pointer to cache entry
  ...
  if (web_files >= web_alloc)
  {
    ...
    if (web_files == 0)
      temp = (hdFileCache *)malloc(sizeof(hdFileCache) * web_alloc);
    else
      temp = (hdFileCache *)realloc(web_cache, sizeof(hdFileCache) * web_alloc);
...
//
// End of "$Id: file.c,v 1.22 2004/10/22 05:43:14 mike Exp $".
//
